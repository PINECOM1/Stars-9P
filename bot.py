“””
Stars 9P — Telegram Bot с Supabase
“””
import logging
import httpx
...
r = await c.get(f”{SB_URL}/rest/v1/players”,
params={“tg_id”: f”eq.{tg_id}”, “select”: “*”}, headers=HEADERS)
rows = r.json()
return rows[0] if rows else None

async def sb_set(data):
async with httpx.AsyncClient() as c:
await c.post(f”{SB_URL}/rest/v1/players”, json=data, headers=HEADERS)
...
if args and args[0].startswith("dep_"):
    ...
    if len(parts) >= 2:
        ...
        await ctx.bot.send_invoice(
            ...
            title=f"Пополнение {amt} ⭐",
            description="Пополнение баланса Stars 9P",
            payload=f"deposit_{user.id}_{amt}",
            provider_token="", currency="XTR",
            prices=[{"label":f"{amt} Stars","amount":amt}])
        return
...
if not p:
await update.message.reply_text(“Напиши /start чтобы начать!”); return
await update.message.reply_text(
f”💼 *Баланс:*\n\n💎 TON: `{float(p.get('ton_balance',0) or 0):.4f}`\n”
f”⭐ Stars: `{int(p.get('stars_balance',0) or 0)}`\n”
f”📊 Депозитов: `{float(p.get('total_deposited',0) or 0):.2f} TON`”,
parse_mode=“Markdown”)
...
if not p:
await update.message.reply_text(“Напиши /start чтобы начать!”); return
g = int(p.get(“games”,0) or 0); w = int(p.get(“wins”,0) or 0)
await update.message.reply_text(
f”📊 *Статистика:*\n\n🎮 Игр: `{g}`\n✅ Побед: `{w}`\n”
f”📈 Винрейт: `{round(w/g*100) if g else 0}%`\n”
f”🏆 Макс: `{float(p.get('max_mult',0) or 0):.2f}×`”,
parse_mode=“Markdown”)
...
text += f”{medals[i] if i<3 else str(i+1)+’.’} {name} — `{int(p.get('stars_balance',0) or 0):,} ⭐`\n”
...
logger.info(“Bot started! Supabase connected.”)
app.run_polling()
